...
export async function updateConcurso(formData: FormData) {
  ...
  await supabase.from('concursos').update({ nome, banca, data_prova: dataProva, prioridades }).eq('id', id);
  revalidatePath('/');
  revalidatePath('/materiais');
}
...
// --- AÇÃO DE RELACIONAMENTO (SEU CÓDIGO - MANTIDO) ---
...
// AÇÃO INTELIGENTE E FLEXÍVEL PARA O CICLO
export async function updateSessaoEstudo(formData: FormData) {
 ...
 // A CORREÇÃO ESTÁ AQUI: ADICIONAMOS A LINHA PARA SALVAR A DISCIPLINA
  if (formData.has('disciplina_id')) {
    const disciplinaId = formData.get('disciplina_id');
    updateData.disciplina_id = disciplinaId ? Number(disciplinaId) : null;
  }

  // Captura todos os campos que podem ter vindo da tabela
...
// AÇÕES PARA LEMBRETES DO CALENDÁRIO (CRUD COMPLETO)
...
// --- AÇÕES PARA DOCUMENTOS (SEU CÓDIGO - MANTIDO) ---
...
// --- AÇÕES PARA TAREFAS (SEU CÓDIGO - MANTIDO) ---
...
// --- AÇÃO PARA ANOTAÇÕES RÁPIDAS (SEU CÓDIGO - MANTIDO) ---
export async function updateAnotacoesRapidas(content: string) {
  const supabase = createServerActionClient({ cookies });
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return;
  await supabase.from('perfis').upsert({ id: user.id, anotações_rapidas: content }, { onConflict: 'id' });
  revalidatePath('/');
}
...
export async function addTopico(formData: FormData) {
  ...
  const conteudo = formData.get('conteudo_rico') as string;
  if (!nome || !disciplinaId) return;
  const supabase = createServerActionClient({ cookies });
  await supabase.from('topicos').insert({ nome, disciplina_id: disciplinaId, conteudo_rico: conteudo });
  revalidatePath('/disciplinas');
}
export async function updateTopico(formData: FormData) {
  const id = Number(formData.get('id'));
  const nome = formData.get('nome') as string;
  const conteudo_rico = formData.get('conteudo_rico') as string;
  if (!id || !nome) return;
  const supabase = createServerActionClient({ cookies });
  await supabase.from('topicos').update({ nome, conteudo_rico }).eq('id', id);
  revalidatePath('/disciplinas');
}
