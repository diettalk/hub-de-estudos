...
const MenuBar = ({ editor }: { editor: any }) => {
  ...
  return (
    <div className="p-2 bg-gray-700 rounded-t-lg flex flex-wrap gap-2">
      ...
      <button type="button" onClick={() => editor.chain().focus().toggleHeading({ level: 2 }).run()} className={getButtonClass('heading')}>
        Título
      </button>
...
export function TextEditor({ value, onChange }: { value: string; onChange: (value: string) => void; }) {
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        // Classes para a área de texto, incluindo cor de fundo e texto
