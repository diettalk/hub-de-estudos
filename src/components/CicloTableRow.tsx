...
export function CicloTableRow({ sessao, disciplinas }: { sessao: SessaoEstudo; disciplinas: Disciplina[] }) {
  ...
  // Usamos um truque com a key do formulário para forçar a reinicialização 
  // do estado quando os dados do servidor mudam.
  const [formKey, setFormKey] = useState(sessao.id); 

  useEffect(() => {
    setFormKey(sessao.id);
  }, [sessao]);
  ...
  return (
    <tr className={`border-b border-gray-700 ${isPending ? 'opacity-50' : ''}`} >
      <td colSpan={11} className="p-0">
        <form ref={formRef} onChange={handleFormChange} key={formKey}>
          <table className="w-full text-sm text-left">
            <tbody>
              <tr className="divide-x divide-gray-700">
                ...
                <td className="p-3 w-12 text-center align-middle"><Checkbox name="concluido" defaultChecked={sessao.concluido} /></td>
                ...
                <td className="p-3 w-1/4 align-middle"><Input name="foco" defaultValue={sessao.foco || ''} className="bg-gray-700" /></td>
                <td className="p-3 w-1/4 align-middle"><Textarea name="diario_de_bordo" defaultValue={sessao.diario_de_bordo || ''} className="bg-gray-700 h-10" /></td>
                ...
                <td className="p-3 w-40 align-middle"><Input name="data_estudo" type="date" defaultValue={sessao.data_estudo || ''} className="bg-gray-700" /></td>
                <td className="p-3 w-40 align-middle"><Checkbox name="r1_concluida" defaultChecked={sessao.r1_concluida} /></td>
                <td className="p-3 w-40 align-middle"><Checkbox name="r2_concluida" defaultChecked={sessao.r2_concluida} /></td>
                <td className="p-3 w-40 align-middle"><Checkbox name="r3_concluida" defaultChecked={sessao.r3_concluida} /></td>
