...
// MenuBar temporariamente desativada para depuração
const MenuBar = ({ editor, onClose }: { editor: Editor | null; onClose: () => void; }) => {
    if (!editor) {
        return null;
    }
    return (
         <div className="p-2 bg-card border-b rounded-t-lg flex justify-end">
...
function TextEditor({ initialContent, onSave, onClose }: TextEditorProps) {
    ...
    return (
        <div className="h-full flex flex-col border rounded-lg bg-card shadow-lg">
            {/* A MenuBar completa está comentada para o teste. Apenas o botão de fechar será mostrado. */}
