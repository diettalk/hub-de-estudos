...
export function CicloTableRow({ sessao, disciplinas }: { sessao: SessaoEstudo; disciplinas: Disciplina[] }) {
  ...
  return (
    <tr className={`border-b border-gray-700 ${sessao.materia_finalizada ? 'bg-black/20 text-gray-500 line-through' : ''} ${isPending ? 'opacity-50' : ''}`}>
      <form action={(formData) => startTransition(() => updateSessaoEstudo(formData))}>
        <td style={{ display: 'contents' }}>
          ...
          {/* CHECKBOX "FINALIZADA" COM AÇÃO IMEDIATA */}
