...
  const contentWidthRef = useRef<HTMLDivElement>(null);
  ...
  // --- NOVA LÓGICA DE PERSISTÊNCIA DE ESTADO ---
  const localStorageKey = `folderState_${table}`;
  ...
  useEffect(() => {
    // Não guardamos o estado inicial vazio para não apagar um estado já salvo
    if (Object.keys(openState).length > 0) {
        localStorage.setItem(localStorageKey, JSON.stringify(openState));
    }
  }, [openState, localStorageKey]);

  // Handler que é chamado pelo react-arborist sempre que uma pasta é aberta/fechada
  const handleToggle = (node: any) => {
    // Atualizamos o nosso mapa de estado
    setOpenState(prev => ({
        ...prev,
