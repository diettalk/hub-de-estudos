...
export function AnotacaoItem({ nota }: { nota: Anotacao }) {
  ...
  const handleFormSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      ...
      setIsEditing(false); // Desativa o modo de edição após a conclusão
    });
  };

  if (isEditing) {
    return (
      <form onSubmit={handleFormSubmit} className="flex justify-between items-center gap-2 bg-gray-900/50 p-3 rounded-md">
