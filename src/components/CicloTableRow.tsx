...
import { deleteSessaoCiclo } from '@/app/actions';

export function CicloTableRow({ sessao, disciplinas }: { sessao: SessaoEstudo; disciplinas: Disciplina[] }) {
  const [isPending, startTransition] = useTransition();

  const handleFormAction = (action: (formData: FormData) => Promise<void>) => (formData: FormData) => {
  ...
  return (
    <tr className={sessao.materia_finalizada ? 'bg-purple-900/30 opacity-40' : sessao.concluido ? 'bg-green-900/30' : 'hover:bg-gray-700/50'}>
      <td className="p-2 text-center"><Checkbox name="concluido" defaultChecked={sessao.concluido} onCheckedChange={(checked) => { const f = new FormData(); f.append('concluido', String(checked)); f.append('data_estudo_was_null', String(!sessao.data_estudo)); handleUpdate(f); }} disabled={isPending} /></td>
      ...
      <td className="p-3 font-bold text-center">{sessao.hora_no_ciclo}</td>
      <td className="p-3">
        <select name="disciplina_id" defaultValue={sessao.disciplina_id || ''} onChange={(e) => { const f = new FormData(); f.append('disciplina_id', e.target.value); handleUpdate(f); }} className="w-full bg-gray-700/50 p-2 rounded-md text-xs border-gray-600">
      ...
      <td className="p-3"><div className="flex items-center gap-1"><Input type="number" name="questoes_acertos" defaultValue={sessao.questoes_acertos || ''} onBlur={(e) => { const f = new FormData(); f.append('questoes_acertos', e.target.value); handleUpdate(f); }} placeholder="A" className="w-14" />/<Input type="number" name="questoes_total" defaultValue={sessao.questoes_total || ''} onBlur={(e) => { const f = new FormData(); f.append('questoes_total', e.target.value); handleUpdate(f); }} placeholder="T" className="w-14" /><span className="w-12 text-center font-bold">{total > 0 ? `${aproveitamento}%` : '-'}</span></div></td>
      <td className="p-3"><Input type="date" name="data_estudo" defaultValue={sessao.data_estudo || ''} onChange={(e) => { const f = new FormData(); f.append('data_estudo', e.target.value); handleUpdate(f); }} className="w-full" /></td>
