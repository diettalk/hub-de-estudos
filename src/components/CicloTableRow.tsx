...
export function CicloTableRow({ sessao, disciplinas }: { sessao: SessaoEstudo, disciplinas: Disciplina[] }) {
  ...
  return (
    <tr className={sessao.materia_finalizada ? 'bg-purple-900/30 opacity-40' : sessao.concluido ? 'bg-green-900/30' : 'hover:bg-gray-700/50'}>
      ...
      <td className="p-3"><select name="disciplina_id" defaultValue={sessao.disciplinas?.id || ''} onChange={(e) => { const f = new FormData(); f.append('disciplina_id', e.target.value); handleUpdate(f);}} className="w-full bg-gray-700/50 p-1 rounded-md text-xs border-gray-600"><option value="">Selecione...</option>{disciplinas.map(d => <option key={d.id} value={d.id}>{d.nome}</option>)}</select></td>
      ...
      <td className="p-3"><div className="flex items-center justify-center gap-2"><Input type="date" name="data_revisao_1" defaultValue={sessao.data_revisao_1 || ''} onBlur={(e) => { const f = new FormData(); f.append('data_revisao_1', e.target.value); handleUpdate(f);}}/><input type="checkbox" name="r1_concluida" defaultChecked={sessao.r1_concluida} onChange={(e) => { const f = new FormData(); f.append('r1_concluida', String(e.target.checked)); handleUpdate(f);}}/></div></td>
      <td className="p-3"><div className="flex items-center justify-center gap-2"><Input type="date" name="data_revisao_2" defaultValue={sessao.data_revisao_2 || ''} onBlur={(e) => { const f = new FormData(); f.append('data_revisao_2', e.target.value); handleUpdate(f);}}/><input type="checkbox" name="r2_concluida" defaultChecked={sessao.r2_concluida} onChange={(e) => { const f = new FormData(); f.append('r2_concluida', String(e.target.checked)); handleUpdate(f);}}/></div></td>
