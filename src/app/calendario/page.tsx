...
type CalendarEvent = {
  ...
  color: string; completed: boolean; revisionType?: 'R1' | 'R7' | 'R30';
  date: Date; data: Lembrete | any;
};
...
export default function CalendarioGeralPage() {
  ...
  const fetchEvents = useCallback(async () => {
    ...
    sessoes?.forEach(r => {
      if (!r.r1_concluida) addEvent(r.data_revisao_1, { key: `r1-${r.id}`, id: r.id, title: r.foco, type: 'revisao', revisionType: 'R1', completed: false, color: '#EF4444', data: r });
      if (!r.r2_concluida) addEvent(r.data_revisao_2, { key: `r2-${r.id}`, id: r.id, title: r.foco, type: 'revisao', revisionType: 'R7', completed: false, color: '#F59E0B', data: r });
      if (!r.r3_concluida) addEvent(r.data_revisao_3, { key: `r3-${r.id}`, id: r.id, title: r.foco, type: 'revisao', revisionType: 'R30', completed: false, color: '#10B981', data: r });
    });
          ...
          <div className="space-y-3 max-h-[45vh] overflow-y-auto pr-2 mb-4">
            {tasksForSelectedDay.length > 0 ? tasksForSelectedDay.map((event) => (
              <div key={event.key} className={`p-3 rounded-lg flex items-center gap-3 border-l-4 ${event.completed ? 'opacity-50' : 'bg-gray-900/50'}`} style={{borderColor: event.color}}>
                {event.type === 'revisao' && <input type="checkbox" className="h-5 w-5 shrink-0" checked={event.completed} onChange={() => startTransition(() => updateRevisaoStatus(event.id, event.revisionType!, !event.completed))} disabled={isPending} />}
