...
// O MenuBar principal continua o mesmo
const MenuBar = ({ editor }: { editor: Editor | null }) => {
    // ... (O código do MenuBar principal não precisa de alterações)
    if (!editor) return null;
    const [highlightColor, setHighlightColor] = useState('#ffcc00');
    const setLink = useCallback(() => { /* ... */ }, [editor]);
    const addYoutubeVideo = useCallback(() => { /* ... */ }, [editor]);
...
function TextEditor({ initialContent, onSave }: TextEditorProps) {
    const editor = useEditor({
        extensions: [
            ...
            Table.configure({ resizable: true }), TableRow, TableHeader, TableCell.extend({
                addAttributes() {
                    return { ...this.parent?.(), backgroundColor: { default: null } };
                },
            }),
            TiptapUnderline,
        ],
    ...
    return (
        <div className="h-full flex flex-col relative">
            ...
            {/* 2. ADICIONAR O BUBBLEMENU PARA A TABELA */}
            {editor && (
                <BubbleMenu 
                    editor={editor} 
                    tippyOptions={{ duration: 100 }} 
                    pluginKey="tableMenu"
                    // Só mostra o menu se o cursor estiver dentro de uma tabela
